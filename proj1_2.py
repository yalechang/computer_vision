...
if flag_s == True:
    filter_box = generate_filter('box',(5,5))
    filter_gaussian = generate_filter('gaussian',(3,3),sigma=s_sigma)
...
for i in range(n_images):
    cv2.imshow('IMG_2',img[i,:,:]*img_mask[i,:,:])
    cv2.waitKey(33)
cv2.destroyWindow('IMG_2')
