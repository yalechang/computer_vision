...
base_path = "/Users/changyale/Dropbox/class_Spring_2014/computer_vision/"+\
        "Project/Project_2/"
...
img_both = np.vstack((img[img_a],img[img_b]))
...
#plt.imshow(img_both,cmap=cm.Greys_r)

# selected corresponding pairs
idx_sel = []
#plt.figure(1)
for i in range(src.shape[0]):
    if cnt[i,0]>n_iter_max*n_sample*1./src.shape[0]/2:
        p1 = (src[i,0],src[i,1])
        p2 = (dst[i,0],dst[i,1])
        cv2.line(img_2,p1,p2,(0,0,0))
        idx_sel.append(i)

#plt.imshow(img_2,cmap=cm.Greys_r)
plt.show()
